...
if __name__ == '__main__':
    ...
    custom_cmap = LinearSegmentedColormap.from_list("purple_green", ["darkgreen", "lightgreen"])
    ...
    ax.plot(time[np.where(np.array(akr)>=0)],akr[np.where(np.array(akr)>=0)],'-',color="lightblue", label="5mW")
    ax.scatter(time[np.where(np.array(opt_akr)>=0)],opt_akr[np.where(np.array(opt_akr)>=0)],color=custom_cmap(1-(opt_power[np.where(np.array(opt_akr)>=0)]-min(opt_power[np.where(np.array(opt_akr)>=0)]))/(max(opt_power[np.where(np.array(opt_akr)>=0)])-min(opt_power[np.where(np.array(opt_akr)>=0)]))), label="P(t)",edgecolor='none',s=3)
    print(f"5mw: total key: {np.trapz(x=time[np.where(np.array(akr)>=0)],y=np.array(akr)[np.where(np.array(akr)>=0)]):.0f}bit, peak rate: {max(akr)}, nonzero time: {time[np.where(np.array(akr)>=0)][-1]-time[np.where(np.array(akr)>=0)][0]}" )
    print(f"optimised: total key: {np.trapz(x=time[np.where(np.array(opt_akr)>=0)],y=np.array(opt_akr)[np.where(np.array(opt_akr)>=0)]):.0f}bit, peak rate: {max(opt_akr)}, nonzero time: {time[np.where(np.array(opt_akr)>=0)][-1]-time[np.where(np.array(opt_akr)>=0)][0]}" )
