...
if __name__ == '__main__':
    ...
    # -0
    loss_behaviour = lambda x,freq,a,b,c,d: a*np.log10(b**2+c**2-2*b*c*np.cos(freq*(x-d))) - 0
    ...
    ax.errorbar(time[np.where(datapoints>0)],datapoints[np.where(datapoints>0)],yerr=yerr[np.where(datapoints>0)],linestyle='',ms=4,marker='o',color="lightblue",markeredgecolor='black', markeredgewidth=1/2, label="-0dB")
    # ax.scatter(time[np.where(datapoints>0)], datapoints[np.where(datapoints>0)], s=4,linewidths=1,marker='o',  color="lightblue", edgecolors='black',zorder=1000) 
    ...
    params = [9.47893858965945,9.89101987420694, 0.037228590019520497, 0.03834132110856429, 6135831.8248959305, 1e-09]
    akr=plot_model(params, powers=[5], loss_range=loss_overpass(time),t_delta=0.4e-9,DC_A=200, DC_B=70, t_dead_A=25e-9, t_dead_B=45e-9)
    ax.plot(time,akr,color="blue")
    datapoints,yerr = plot_data(run_name,loss_overpass(time), 5, time)
    ax.errorbar(time[np.where(datapoints>0)],datapoints[np.where(datapoints>0)],yerr=yerr[np.where(datapoints>0)],linestyle='',ms=4,marker='o', color="blue",markeredgecolor='black', markeredgewidth=1/2, label="-3dB")
    # ax.scatter(time[np.where(datapoints>0)], datapoints[np.where(datapoints>0)], s=4,linewidths=1,marker='o',  color="blue", edgecolors='black',zorder=1000) 
    print(f"total key: {np.trapz(x=time[np.where(np.array(akr)>=0)],y=np.array(akr)[np.where(np.array(akr)>=0)]):.0f}bit, peak rate: {max(akr)}, nonzero time: {time[np.where(np.array(akr)>=0)][-1]-time[np.where(np.array(akr)>=0)][0]}" )
    ...
    datapoints,yerr = plot_data(run_name,loss_overpass(time), 5, time)
    ax.errorbar(time[np.where(datapoints>0)],datapoints[np.where(datapoints>0)],yerr=yerr[np.where(datapoints>0)],linestyle='',ms=4,marker='o',color="darkblue",markeredgecolor='black', markeredgewidth=1/2, label="-8.9dB")
    # ax.scatter(time[np.where(datapoints>0)], datapoints[np.where(datapoints>0)], s=4,linewidths=1,marker='o',  color="darkblue", edgecolors='black',zorder=1000) 
    print(f"total key: {np.trapz(x=time[np.where(np.array(akr)>=0)],y=np.array(akr)[np.where(np.array(akr)>=0)]):.0f}bit, peak rate: {max(akr)}, nonzero time: {time[np.where(np.array(akr)>=0)][-1]-time[np.where(np.array(akr)>=0)][0]}" )
